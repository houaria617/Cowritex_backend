...
class LiteratureAgent:
    ...
    def load_documents(self, input_data: AgentInput) -> bool:
        ...
        self.vector_store.build_index(self.chunks, use_hnsw=len(self.chunks) > 1000)
        return True
    ...
    def semantic_verification(self, claim: str, source_chunks: List[str], threshold_weak: float = 0.55, threshold_unverified: float = 0.45) -> Tuple[str, float, dict]:
        ...
        similarities = np.dot(chunk_embs, claim_emb) / (np.linalg.norm(chunk_embs, axis=1) * np.linalg.norm(claim_emb))
    ...
    def generate_review(self, input_data: AgentInput) -> AgentOutput:
        try:
            ...
            if "## References" in review_for_verification:
                review_for_verification = review_for_verification.split("## References")[0]
            if "References" in review_for_verification:
                review_for_verification = review_for_verification.split("References")[0]

            # -------- Semantic Verification --------
            raw_sentences = [s.strip() for s in review_for_verification.replace('\n', ' ').split('.') if len(s.strip()) > 30]
            claim_sentences = [s for s in raw_sentences if not re.match(r'^\d+(\.\d+)*\s*$', s)]
            ...
            for sent in claim_sentences[:25]:
                status, conf, best = self.semantic_verification(sent, source_texts)
                result = {
...
if __name__ == "__main__":
    import sys
    title = sys.argv[1] if len(sys.argv) > 1 else input("Enter project title: ")
